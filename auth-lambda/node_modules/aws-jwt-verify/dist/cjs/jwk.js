...
const parseJwks = function (jwksBin) {
    ...
    try {
        ...
        jwks = (0, safe_json_parse_js_1.safeJsonParse)(jwksText);
    }
    catch (err) {
        throw new error_js_1.JwksValidationError(`JWKS could not be parsed as JSON: ${err}`);
    }
    assertIsJwks(jwks);
    return jwks;
};
async function fetchJwks(jwksUri) {
    return (0, https_js_1.fetch)(jwksUri).then(parseJwks);
}
...
function assertIsSignatureJwk(jwk) {
    (0, assert_js_1.assertStringArrayContainsString)("JWK kty", jwk.kty, ["EC", "RSA", "OKP"], error_js_1.JwkInvalidKtyError);
    if (jwk.kty === "EC") {
...
function assertIsEdDSASignatureJwk(jwk) {
    ...
    // Check JWK kty
    (0, assert_js_1.assertStringEquals)("JWK kty", jwk.kty, "OKP", error_js_1.JwkInvalidKtyError);
    // Check Curve (crv) has a value
    if (!jwk.crv)
        throw new error_js_1.JwkValidationError("Missing Curve (crv)");
    // Check X Coordinate (x) has a value
    if (!jwk.x)
        throw new error_js_1.JwkValidationError("Missing X Coordinate (x)");
}
function assertIsEsSignatureJwk(jwk) {
    ...
    // Check JWK kty
    (0, assert_js_1.assertStringEquals)("JWK kty", jwk.kty, "EC", error_js_1.JwkInvalidKtyError);
    // Check Curve (crv) has a value
    if (!jwk.crv)
        throw new error_js_1.JwkValidationError("Missing Curve (crv)");
    // Check X Coordinate (x) has a value
    ...
    if (!jwk.y)
        throw new error_js_1.JwkValidationError("Missing Y Coordinate (y)");
}
